...
function hexToHsb(hex){var p=parseHex(hex)||"#00509d",v=p.slice(1),rn=parseInt(v.slice(0,2),16)/255,gn=parseInt(v.slice(2,4),16)/255,bn=parseInt(v.slice(4,6),16)/255,mx=Math.max(rn,gn,bn),mn=Math.min(rn,gn,bn),d=mx-mn,h=0;if(d){if(mx===rn)h=((gn-bn)/d)%6;else if(mx===gn)h=(bn-rn)/d+2;else h=(rn-gn)/d+4;h*=60;if(h<0)h+=360;}var s=mx===0?0:(d/mx)*100;return{h:h,s:s,b:mx*100};}
...
function derive(hex,dark){var hsb=hexToHsb(hex),h=hsb.h,s=hsb.s,b=hsb.b;if(s<12)return dark?{accent:"#d1d1d6",bright:"#ebebf0"}:{accent:"#3a3a3c",bright:"#636366"};if(dark)return{accent:hsbToHex(h,clamp(s,42,100),clamp(b,52,82)),bright:hsbToHex(h,clamp(s*0.88,32,100),clamp(b+16,68,94))};return{accent:hsbToHex(h,clamp(s,48,100),clamp(b*0.52,26,54)),bright:hsbToHex(h,clamp(s*0.92,36,100),clamp(Math.max(b,50)*0.72,42,70))};}
...
function resolveAccent(){var raw=localStorage.getItem(ak);var hex=parseHex(raw);if(hex)return hex;var legacy=localStorage.getItem(hk);if(legacy&&hues.indexOf(legacy)>=0&&presetHex[legacy])return presetHex[legacy];if(legacy&&presetHex[legacy])return presetHex[legacy];return presetHex[defHue]||"#00509d";}
