...
/** Iframe src: collaborative editor chrome reduced (rm=minimal) + embedded mode. */
export function googleDocsEmbeddedEditSrc(documentId: string): string {
  return `https://docs.google.com/document/d/${encodeURIComponent(documentId)}/edit?embedded=true&rm=minimal`;
}
