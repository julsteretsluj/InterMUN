...
BEGIN
  ...
  SELECT rce.attendance
  INTO v_existing_attendance
  FROM public.roll_call_entries rce
  WHERE rce.conference_id = p_conference_id
    AND rce.allocation_id = v_allocation_id;

  IF v_existing_attendance IN ('present_abstain', 'present_voting') THEN
    RAISE EXCEPTION 'roll-call attendance already set and cannot be changed';
  END IF;
