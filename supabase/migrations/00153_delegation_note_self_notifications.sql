-- Allow delegation-note notifications when the recipient is the sender (testing / self-notes).
...
BEGIN
  ...
  ELSIF NEW.recipient_kind = 'chair' AND NEW.recipient_profile_id IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = NEW.recipient_profile_id) THEN
      INSERT INTO public.user_notifications (
        user_id, conference_id, type, title, body, href, reference_id
      ) VALUES (
