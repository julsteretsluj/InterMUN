-- Optional Google Slides URL for crisis committees (embedded in-app at /crisis-slides).

BEGIN;

ALTER TABLE public.conferences
  ADD COLUMN IF NOT EXISTS crisis_slides_url text;

COMMENT ON COLUMN public.conferences.crisis_slides_url IS
  'Google Slides link (docs.google.com/presentation/d/…); shown embedded for FWC / UNSC / HSC when set.';

DROP FUNCTION IF EXISTS public.update_committee_session_smt(uuid, text, text, text, text, text, text);
...
REVOKE ALL ON FUNCTION public.update_committee_session_smt(uuid, text, text, text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.update_committee_session_smt(uuid, text, text, text, text, text, text, text) TO authenticated;
