...
import {
  startCommitteeSessionAction,
  stopCommitteeSessionAction,
...
export function ChairCommitteeSessionControl({
  ...
  const glassInput =
    "rounded-xl border border-[var(--hairline)] bg-[var(--material-thin)] px-3 py-2 text-sm text-brand-navy placeholder:text-brand-muted/60 focus:border-brand-accent/40 focus:outline-none focus:ring-2 focus:ring-brand-accent/25";
      ...
      <div className="rounded-2xl border border-[var(--hairline)] bg-[var(--dashboard-card)] p-6 text-brand-navy shadow-sm backdrop-blur-sm md:p-8">
        ...
        <div className="mt-4 space-y-1.5">
          ...
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-brand-muted">
            ...
            {titleSaveStatus === "saving" ? (
              <span className="text-brand-navy/85">{t("sessionNameSaving")}</span>
            ) : null}
        ...
        <div className="mt-6 space-y-4 rounded-xl border border-[var(--hairline)] bg-[var(--material-thin)] p-4">
          ...
          <p className="text-xs text-brand-muted">
            {t("sessionLimitOptional")} <strong className="font-medium text-brand-navy">{t("noneLabel")}</strong>{" "}
            {t("noneDescription")} <strong className="font-medium text-brand-navy">{t("timeBudgetLabel")}</strong>{" "}
            {t("timeBudgetDescription")} <strong className="font-medium text-brand-navy">{t("endTimeLabel")}</strong>{" "}
