...
export function ChairHowToAccordion() {
  ...
  return (
    <div className="rounded-xl border border-[var(--hairline)] bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900/80">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
        className="flex w-full items-center justify-between gap-2 px-4 py-3 text-left text-sm font-semibold text-brand-navy dark:text-zinc-100"
      >
        How to use this system (Chair)
        <ChevronDown
          className={cn("h-5 w-5 shrink-0 text-brand-muted transition-transform", open && "rotate-180")}
          strokeWidth={1.75}
          aria-hidden
        />
      </button>
      {open ? (
        <div className="border-t border-[var(--hairline)] px-4 py-3 text-sm leading-relaxed text-brand-navy dark:border-zinc-700 dark:text-zinc-300">
          <p>
          ...
          <ul className="mt-3 list-disc space-y-1 pl-5">
            <li>
              <strong className="font-medium text-brand-navy dark:text-zinc-100">Digital Room</strong> — placard list
              with roll status plus private compliment/concern/reminder flags (saved in this browser).
            </li>
            <li>
              <strong className="font-medium text-brand-navy dark:text-zinc-100">Delegates</strong> — use the matrix to
            ...
            <li>
              <strong className="font-medium text-brand-navy dark:text-zinc-100">Voting</strong> — delegate-facing vote
