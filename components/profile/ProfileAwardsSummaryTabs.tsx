...
/**
 * Pending vs recorded awards. Sub-tabs render only when both sections have content.
 */
export function ProfileAwardsSummaryTabs({
  ...
  if (hasPending && hasRecorded) {
    return (
      <div className="mb-8 space-y-4">
        ...
        <div
          role="tabpanel"
          aria-labelledby={tab === "pending" ? "tab-profile-pending-noms" : "tab-profile-recorded-awards"}
        >
  ...
  if (hasPending) {
    return <div className="mb-8 space-y-4">{pendingSlot}</div>;
  }
  if (hasRecorded) {
    return <div className="mb-8 space-y-4">{recordedSlot}</div>;
  }
  return null;
}
