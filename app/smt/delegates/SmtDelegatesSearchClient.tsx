...
export function SmtDelegatesSearchClient({ rows }: { rows: SmtDelegateSearchRow[] }) {
  ...
  return (
    <ChairDelegateProfileSheetProvider>
      <div className="mb-5">
        ...
        <div className="relative max-w-xl">
          ...
          <input
            ...
            className="w-full rounded-xl border border-[#C9DDE9] bg-white py-2.5 pl-10 pr-3 text-sm text-brand-navy outline-none ring-[#119ED3] placeholder:text-brand-muted focus:ring-2 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
          />
        </div>
      ...
      {rows.length === 0 ? (
        <p className="rounded-xl border border-[#C9DDE9] bg-white px-4 py-8 text-center text-sm text-brand-muted dark:border-zinc-700 dark:bg-zinc-900/50">
          {t("empty")}
        </p>
      ) : filtered.length === 0 ? (
        <p className="rounded-xl border border-[#C9DDE9] bg-white px-4 py-8 text-center text-sm text-brand-muted dark:border-zinc-700 dark:bg-zinc-900/50">
          ...
          {filtered.map((row) => {
            ...
            return (
              <li key={row.allocationId}>
                <ChairDelegateProfileTrigger
                  delegate={row}
                  className="flex h-full w-full items-center gap-3 rounded-[var(--radius-md)] border border-[#C9DDE9] bg-[color-mix(in_srgb,#F8FBFF_88%,white)] px-4 py-3 text-left shadow-[var(--dashboard-shadow)] transition-[transform,box-shadow] duration-[var(--dur-base)] hover:-translate-y-0.5 hover:shadow-[var(--dashboard-shadow-hover)] dark:border-zinc-700 dark:bg-zinc-900/60"
                >
                  <ChairDelegateAvatar
                    name={displayName}
                  ...
                  <span className="min-w-0">
                    ...
                    {row.email ? (
                      <span className="mt-0.5 block truncate text-[0.7rem] text-[#4D6174] dark:text-zinc-500">
                        {row.email}
                      </span>
                    ) : null}
                  </span>
