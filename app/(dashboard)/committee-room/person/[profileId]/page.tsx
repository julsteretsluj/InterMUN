...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-brand-navy/15 bg-white px-3 py-2.5 dark:border-white/12 dark:bg-black/25">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 sm:justify-end">
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 rounded-full border border-brand-accent/35 bg-brand-accent/14 px-4 py-2 text-sm font-semibold text-brand-navy shadow-sm hover:opacity-95 dark:text-zinc-100"
          >
        ...
        <aside className="rounded-xl border border-brand-navy/15 bg-brand-paper p-5 shadow-[0_10px_28px_-16px_rgba(0,0,0,0.45)] dark:border-white/12 dark:bg-black/20">
          ...
          <div className="mt-5 space-y-3 border-t border-brand-navy/15 pt-5 dark:border-white/10">
            ...
            >
              <Link
                href={`/chats-notes?forProfile=${encodeURIComponent(profileId)}`}
                className="flex flex-col items-center gap-1.5 rounded-xl border border-brand-navy/15 bg-white py-3 text-xs font-semibold text-brand-navy transition-colors hover:border-brand-accent/32 dark:border-white/12 dark:bg-black/20 dark:text-zinc-100"
              >
              ...
              {crisisReportingEnabled ? (
                <Link
                  href={`/report?about=${encodeURIComponent(profileId)}&aboutName=${reportAboutName}`}
                  className="flex flex-col items-center gap-1.5 rounded-xl border border-brand-navy/15 bg-white py-3 text-xs font-semibold text-brand-navy transition-colors hover:border-rose-400/35 dark:border-white/12 dark:bg-black/20 dark:text-zinc-100"
                >
        ...
        <div className="space-y-4 min-w-0">
          {placard || (canReadFullProfile && fullProfile) ? (
            <section className="rounded-xl border border-brand-navy/15 bg-brand-paper p-5 shadow-[0_10px_24px_-14px_rgba(0,0,0,0.35)] dark:border-white/12 dark:bg-black/20 md:p-6">
              ...
              {placard && canReadFullProfile && fullProfile ? (
                <div className="my-4 border-t border-brand-navy/15 dark:border-white/10" />
              ) : null}
